...
async def show_about(message: Message) -> None:
    """Показывает информацию о мастере из БД."""
    about_text = await db.get_content("about")
    
    if not about_text:
        about_text = "ℹ️ Раздел 'Обо мне' пока не заполнен."
    
    about_photo = config.about_photo_url
...
async def show_memo(message: Message) -> None:
    """Показывает памятку для клиентов из БД."""
    memo_text = await db.get_content("memo")
    
    if not memo_text:
        memo_text = "📋 Раздел 'Памятка для клиентов' пока не заполнен."
...
async def show_contacts(message: Message) -> None:
    """Показывает контакты из БД."""
    contacts_text = await db.get_content("contacts")
    
    if not contacts_text:
        contacts_text = "📞 Раздел 'Контакты' пока не заполнен."
